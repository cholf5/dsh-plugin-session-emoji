...
| 🖱️ | 右键设置 | 会话行右键 → 「设置 emoji…」打开选择器（190 个常用 emoji），「清除 emoji」一键移除 |
...
> 在 dsh `0.1.5-rc.2` 上开发验证。

**① 把插件放进 web profile 的 `node_modules`**

```sh
git clone git@github.com:cholf5/dsh-plugin-session-emoji.git
ln -s "$PWD/dsh-plugin-session-emoji" ~/.dsh/profiles/web/node_modules/dsh-plugin-session-emoji
```

<details>
<summary>或者：正式安装（pnpm 托管）</summary>

```sh
dsh plugin --profile web add /path/to/dsh-plugin-session-emoji
```

</details>

**② 在 `~/.dsh/profiles/web/cordis.patch.yml` 里启用**
...
保存即生效（`patchReload: live` 会热加载 Loader 条目），**无需重启 dsh web**。

**③ 刷新浏览器页面** —— 浏览器插件 roster 在页面加载时注入。
...
| `lib/index.js`（Host 半） | 轻触一下 `cordis.patch.yml`（加个空行保存）触发 patch 重载，或重启 `dsh web` |

> symlink 安装方式下，直接编辑仓库源码即可，无需重新安装。
...
```sh
# 1. 从 ~/.dsh/profiles/web/cordis.patch.yml 删掉 insert 条目
# 2. 删除软链/安装
rm ~/.dsh/profiles/web/node_modules/dsh-plugin-session-emoji
# 3.（可选）删除数据
